...
7b. **Seguridad (condicional):** si el diff toca `n8n/workflows/` (nodos Build Prompt/Claude/Anthropic), `scripts/agent/`, `.claude/`, `.github/workflows/`, o RPCs/SQL que alimentan prompts (analytics/`get_memoria_activa`/E5*), delega en `security-reviewer` y exige `SEC-VEREDICTO: PASS` anclado al MISMO `sha` que el reviewer ANTES del gate. Un `FAIL` se trata como `REQUEST_CHANGES` (→ `fixer`, mismo contador de intentos `AIR-<n>-verify`). Si el diff no toca esa superficie, salta este paso.
...
- Telemetría: si el humano pide estado de la flota, corre `bash scripts/agent/fleet-metrics.sh`.
