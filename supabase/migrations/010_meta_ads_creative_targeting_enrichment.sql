-- Migration: 010_meta_ads_creative_targeting_enrichment
-- Date: 2026-04-27
-- Purpose: Tier 1 (creative) + Tier 2 (targeting) enrichment for Meta Ads embeddings
-- Adds: link_description, image_url, video_id, asset_feed_*, optimization_goal,
--       targeting_summary, targeting_raw
-- Updates: upsert_meta_ads RPC to handle new fields with COALESCE-based merge
--          (preserves existing values if new pull returns null)

ALTER TABLE meta_ads_performance
  ADD COLUMN IF NOT EXISTS link_description text,
  ADD COLUMN IF NOT EXISTS image_url text,
  ADD COLUMN IF NOT EXISTS video_id text,
  ADD COLUMN IF NOT EXISTS asset_feed_titles text[],
  ADD COLUMN IF NOT EXISTS asset_feed_bodies text[],
  ADD COLUMN IF NOT EXISTS asset_feed_descriptions text[];

ALTER TABLE meta_ads_performance
  ADD COLUMN IF NOT EXISTS optimization_goal text,
  ADD COLUMN IF NOT EXISTS targeting_summary text,
  ADD COLUMN IF NOT EXISTS targeting_raw jsonb;
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO meta_ads_performance (
      ...
      ad->>'image_url',
      ad->>'video_id',
      CASE WHEN ad ? 'asset_feed_titles' THEN ARRAY(SELECT jsonb_array_elements_text(ad->'asset_feed_titles')) ELSE NULL END,
      CASE WHEN ad ? 'asset_feed_bodies' THEN ARRAY(SELECT jsonb_array_elements_text(ad->'asset_feed_bodies')) ELSE NULL END,
      CASE WHEN ad ? 'asset_feed_descriptions' THEN ARRAY(SELECT jsonb_array_elements_text(ad->'asset_feed_descriptions')) ELSE NULL END,
      ad->>'optimization_goal',
      ad->>'targeting_summary',
      CASE WHEN ad ? 'targeting_raw' THEN (ad->'targeting_raw') ELSE NULL END
    )
