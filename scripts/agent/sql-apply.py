...
CÓDIGOS DE SALIDA (el gate los distingue; no son intercambiables):
  ...
  3  no se pudo CONECTAR — no es culpa de la migración, y decirlo importa:
     ...
     culpaba a la migración de una caída de la base. OJO: `3` identifica el
     MOMENTO (falló en connect()), no la causa. Ahí caben al menos red/puerto
     ("Connection refused"), AUTENTICACIÓN ("password authentication failed")
     y base inexistente. Quien lo consuma no debe afirmar una de las tres: el
     detalle de libpq que se imprime dice cuál fue.
...
try:
    ...
    sys.stderr.write(
        "sql-apply: no se pudo importar psycopg2.\n"
        "Lo normal es que no esté instalado (Debian/Ubuntu: python3-psycopg2),\n"
        "pero OJO: el gate invoca este script con `python3 -I`, que ignora\n"
        "PYTHONPATH y el site-packages del USUARIO — así que un\n"
        "`pip install --user psycopg2-binary` tampoco se vería desde aquí.\n"
