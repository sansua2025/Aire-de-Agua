...
>
> **Actualización 2026-07-05 (2):** Paso 0 **COMPLETO** — decisiones D1–D5 cerradas en `docs/adr/ADR-004-pnl-decisiones-semanticas.md` (verificación read-only contra PROD + reconciliación al peso contra el oráculo VP); la mig 115 (Paso 1) queda **desbloqueada** tras aprobación del ADR.

| # | Verificación | Cómo | Resuelve |
|---|---|---|---|
| 0.1 | ~~¿`total_linea` es neto o bruto de descuento?~~ **RESUELTA:** `total_linea` es GENERATED `(precio_unitario − descuento) × cantidad` → neto del descuento de LÍNEA, bruto del de ORDEN; `ventas.descuento` (header) incluye ambos; identidad `subtotal = Σ(pu×qty) − descuento` exacta en ~98% (1.474/1.505); descuentos de línea raros (~1,6%). El Bruto parte de `Σ(pu×qty)`, no de `Σ total_linea` (restar `ventas.descuento` a este duplicaría) | Línea Descuentos |
| 0.2 | ~~Semántica de `ventas.costo_envio` e `impuesto`~~ **RESUELTA:** `total = subtotal + costo_envio` en 100% (1.505/1.505) → `costo_envio` es lo COBRADO al cliente (carrier NO capturado → gasto tipo Shipping); `impuesto = subtotal × 19/119` (IVA-incluido 19%, envío no gravado, informativo) → **D1 cerrada** | Línea Envío cobrado |
| 0.3 | ~~¿`E2_Webhook_Shopify_Orders` procesa refunds?~~ **RESUELTA (repo):** el workflow solo escucha `orders/create` y `orders/updated`; no hay topic de refunds. El upsert (mig 007) sí actualiza `estado_pago` on-conflict, así que un refund llega como `orders/updated` y la orden pasa a `refunded`/`partially_refunded`. **Consecuencia:** con el filtro `estado_pago='paid'`, hoy un refund TOTAL borra la orden del revenue retroactivamente (en el mes de la ORDEN, no del refund — contra la política 1 de VP) y un refund PARCIAL elimina la orden COMPLETA (sobre-corrige). **Conteo confirmado en PROD:** 18 órdenes all-time en estados refund (15 `refunded` + 3 `partially_refunded`), solo 2 en 2026 (~0,2% del revenue 2026) | Diseño del paso 2 |
...
| 0.5 | ~~¿`ventas_offline` solapa con `ventas.canal='pos'`?~~ **RESUELTA:** `ventas_offline` está VACÍA (0 filas), sin solape posible con las 1.120 órdenes pos paid → **D5 cerrada:** alcance = `ventas` (web+pos+draft) paid; `ventas_offline` queda fuera, documentada como tabla dormida | D5 (alcance de ventas) |
| 0.6 | ~~Cobertura COGS actual del período a validar~~ **RESUELTA:** 2026 = 99,83% de unidades con COGS (~100% del revenue; 1 línea sin COGS de 602); `vista_atribucion_web_con_margen` 100% cobertura 'completa' → calidad suficiente para reportar sin fallback | Calidad esperada del margen |
| 0.7 | ~~Solape pauta: pauta devengada vs `gastos` publicidad/agencia~~ **RESUELTA:** la categoría `Publicidad` es caja de la MISMA pauta Meta (conceptos "Pauta Meta &lt;mes&gt;", dejó de registrarse desde abr-2026) → **D2 cerrada:** Publicidad del P&L sale de `meta_ads_performance.gasto`; del OPEX se excluyen `Publicidad`/`COGS`/`Assets`; entran `Agencia`, Feria, Fotos, Influencers, Marketing Automation y operativas | D2 (qué categorías se excluyen del OPEX) |
| 0.8 | ~~**Reconciliación cruzada AdeA↔VP**~~ **RESUELTA:** ventana 290 órdenes (conteo AdeA = 290, exacto); dic/ene/feb/mar cuadran al peso en bruto, descuentos, refunds y COGS orden a orden salvo 2 residuos declarados (COGS ene ~1,4%; refund mar irreconstruible → gap del Paso 2); `get_revenue` ene/feb = `Σ total_linea` paid (55/52 órdenes) → confirmada como ventas BRUTAS | Confianza en ambas fuentes antes de construir |
