...
| 0.3 | ~~¿`E2_Webhook_Shopify_Orders` procesa refunds?~~ **RESUELTA (repo):** el workflow solo escucha `orders/create` y `orders/updated`; no hay topic de refunds. El upsert (mig 007) sí actualiza `estado_pago` on-conflict, así que un refund llega como `orders/updated` y la orden pasa a `refunded`/`partially_refunded`. **Consecuencia:** con el filtro `estado_pago='paid'`, hoy un refund TOTAL borra la orden del revenue retroactivamente (en el mes de la ORDEN, no del refund — contra la política 1 de VP) y un refund PARCIAL elimina la orden COMPLETA (sobre-corrige). Queda por confirmar en PROD el conteo de órdenes en esos estados | Diseño del paso 2 |
