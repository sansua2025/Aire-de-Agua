...
2. **El preview branch lo crea la integración de GitHub, NO el agente.** Supabase levanta **un preview branch por PR** y corre ahí las migraciones solo: ese es el check `Supabase Preview` del PR, y **ese** es el mecanismo que satisface "preview branch primero". Flujo correcto: migración nueva en `supabase/migrations/` → PR → check `Supabase Preview` en verde → merge → aplicar a PROD con confirmación humana. **Nunca `create_branch` vía MCP**, por tres motivos concretos: (a) consume el cupo de branches concurrentes y, al agotarlo, el check automático del PR **se salta en silencio** (el bot comenta *"ignored … due to reaching the limit of concurrent preview branches"*); (b) cuesta ~US$0,0134/hora mientras el branch viva; (c) el grant OAuth del MCP **no alcanza los proyectos de preview** (no aparecen en `list_projects` y todo `execute_sql` contra su ref devuelve *"You do not have permission to perform this action"*), así que el agente no puede validar nada ahí aunque lo cree.
   ...
   - **Refuerzo:** el hook `scripts/agent/hooks/guard-prod-writes.sh` (PreToolUse) pide confirmación en todo `apply_migration` y en `execute_sql` **cuya query contenga un verbo de write** (`INSERT|UPDATE|DELETE|CREATE|ALTER|DROP|GRANT|REVOKE|TRUNCATE`, como palabra). La detección es por verbo SQL, así que **NO cubre** los writes vía RPC (`select <función_que_escribe>(...)` — p.ej. `select public.ingest_refund(...)`, que en este repo es la vía canónica) ni `MERGE`/`CALL`/`COPY`/`REFRESH MATERIALIZED VIEW`/`setval`/`LOCK TABLE`: todos esos pasan EN SILENCIO. Ver la sección `LÍMITE CONOCIDO` de la cabecera del hook antes de fiarte de él. Hace matching por **SUFIJO ANCHO** del tool (`*apply_migration*`, `*execute_sql*`), no por el literal `mcp__supabase__*` — ese literal falló ABIERTO porque el prefijo del servidor MCP varía según el entorno (en remoto llega como `mcp__<uuid>__execute_sql`). Cubierto por `scripts/agent/hooks/guard-prod-writes.test.sh`. `scripts/agent/check-guard-coverage-parity.sh` (CI `hooks-guards`) sostiene esa cobertura **ejecutando el hook**, no leyendo su texto: le manda por stdin un `tool_name` sintético por cada tool del piso —con tres prefijos de servidor distintos, uno de ellos con UUID como el del entorno remoto, y con la variante `<tool>_v2`— y exige que responda `ask`; comprueba además que un `SELECT` puro y un tool ajeno sigan pasando en silencio, para que ese verde signifique algo. La capa 1 (`.claude/settings.json`) no se puede ejecutar: de ahí lee con `jq` los `matcher` cuyo `command` **nombra** el hook y **evalúa la regex de verdad** contra esos mismos nombres sintéticos, reproduciendo la semántica del binario (un matcher sin metacaracteres se compara por igualdad exacta) — por eso el literal `mcp__supabase__execute_sql` falla y un espacio dentro del grupo también. De esta sección comprueba dos cosas, ambas verificables: que la verb-list de arriba esté citada aquí literal, y que todo glob `*<tool>*` que se cite corresponda a un tool que el hook frena de verdad. Lo que NO hace: validar la prosa, ni comprobar que el `command` cableado llegue a ejecutar el script (solo que lo nombre), ni sustituir a `guard-prod-writes.test.sh`, que es quien cubre el resto de la semántica. La cobertura declarada divergió de la real 3 veces en una sola ronda de revisión de AIR-162 (PR #184) antes de graduarse a este check.
