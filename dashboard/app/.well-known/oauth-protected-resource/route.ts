 ...
 * el `authorization_servers` (= issuer de Descope) y desde ahi hace DCR/PKCE/token
 * contra Descope (RFC 8414). NUESTRO servidor NO es el AS.
 *
...
// El issuer del AS se resuelve en request-time. Si falta DESCOPE_PROJECT_ID,
// getOAuthProvider() lanza ⇒ 500 (preferible a anunciar un AS vacio/invalido).
function buildHandler() {
  const { issuer } = getOAuthProvider()
  return protectedResourceHandler({ authServerUrls: [issuer] })
}
